...
g1 <- ggplot(tmp, aes(y=motif_cluster, x=cell_type, fill=p.summary)) + 
    ...
    xlab("Gene Trend")
ggsave(g1, file="supp_figures/SuppFig7_enrichment_gene_trends.svg", height=6, width=4)
...
g1 <- all_tfs %>% filter(padjust<0.05) %>%
    ...
    ylab("Motif") +
    xlab("Gene Trend")
ggsave(g1, file="supp_figures/SuppFig7_enrichment_gene_trends_full.svg", height=12, width=4)
...
g1 <- all_tfs[all_tfs$feature_short %in% subset_motifs,] %>% 
    ...
    ylab("Motif") +
    xlab("Gene Trend")
ggsave(g1, file="supp_figures/SuppFig7_enrichment_gene_trends_subset_full.svg", height=12, width=4)
