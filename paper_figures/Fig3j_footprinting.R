...
plot_footprint <- function(seFoot, seFoot_ctrl, motif){
    ...
    plotFootDF <- data.frame(
        x = footDF$x, 
        mean = footMatMean, footMatMean_ctrl, 
        sd = footMatSd, footMatSd_ctrl
    )
    ...
    geom_line() + geom_line(data=plotFootDF_ctrl,
