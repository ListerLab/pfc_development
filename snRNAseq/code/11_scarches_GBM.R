...
query = readRDS('snRNAseq/processed_data/transfer_learning/datasets/gbm_primary_dataset.RDS') 
...
saveRDS(latent, 'snRNAseq/processed_data/transfer_learning/outs/GBM_latent.RDS')
saveRDS(qlatent, 'snRNAseq/processed_data/transfer_learning/outs/GBM_qlatent.RDS')
...
out = myknn(ref_embed = refmap[,1:2], test_embed = qumap[,1:2], 
            test_class = qumap$cluster, ref_class = reference$cluster, k =10)
qumap$result = out$result


saveRDS(qumap, file="snRNAseq/processed_data/GBM_umap.RDS")
