...
for(i in 1:length(cell_types)){
    ...
    geom_text_repel(data=df_i[df_i$betweeness>200&df_i$hub>0.5,], 
                    aes(label=gene)) 
    ggsave(inf_gg, file=paste0("supp_figures/SuppFig4_Network_", cell_types[i],
    ...
    g1 <- ggraph(g_a, layout = "centrality", cent=degree(g_a)) + 
        ...
        geom_node_text(aes(label = name, filter=degree(g_a)>10), 
                       size = 5, repel = TRUE) +
        theme_graph() 
    ggsave(g1, file=paste0("supp_figures/SuppFig4_Network_", cell_types[i],
    ...
    geom_text_repel(data=df_a[df_a$betweeness>200&df_a$hub>0.5,], 
                    aes(label=gene)) 
    ggsave(a_gg, file=paste0("supp_figures/SuppFig4_Network_",
